...
class TestQuoteRouters:
    ...
    def test_get_latest_quote_validation(self):
        ...
        response = client.get("/v1/quotes//latest")
        assert response.status_code in [404, 422]
        
        # Very long ticker should fail validation or return error
        response = client.get("/v1/quotes/VERYLONGTICKER123/latest")
    ...
    def test_get_history_structure(self):
        ...
        response = client.post("/v1/quotes/AAPL/history", json=request_data)
    ...
    def test_batch_quotes_structure(self):
        """Test batch quotes GET endpoint exists."""
        request_data = {
            "tickers": ["AAPL", "GOOGL", "MSFT"]
        }
        
        # GET with body (non-standard but supported by FastAPI)
        response = client.request("GET", "/v1/quotes/batch", json=request_data)
    ...
    def test_list_tickers_pagination(self):
        ...
        response = client.get("/v1/quotes/?limit=50&offset=10")
    ...
    def test_list_tickers_validation(self):
        ...
        response = client.get("/v1/quotes/?limit=-1")
        assert response.status_code == 422
        
        # Limit too large should fail
        response = client.get("/v1/quotes/?limit=2000")
        assert response.status_code == 422
        
        # Negative offset should fail
        response = client.get("/v1/quotes/?offset=-5")
        assert response.status_code == 422
...
class TestAPIDocumentation:
    ...
    def test_quotes_endpoints_in_docs(self):
        ...
        assert "/v1/quotes/{ticker}/latest" in paths
        assert "/v1/quotes/{ticker}/history" in paths
        assert "/v1/quotes/batch" in paths
